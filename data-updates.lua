    ...
    if game.active_mods["train-trails"] then
        ...
        local function get_rainbow_color(created_tick, mod_settings, train_data)
            ...
            elseif animation_colors then
                local index = floor(modifier %
                (mod_settings.animation_color_count or train_data.random_animation_colors_count)) + 1
        ...
        local function draw_trails(event_tick, mod_settings)
            ...
            if mod_settings.passengers_only then
                for _, player in pairs(game.connected_players) do
                    local train_data = player.vehicle and player.vehicle.train and
                    active_train_datas[player.vehicle.train.id]
                    if train_data then
